...
//               в строке ниже составляем путь копирования - /путь до папки в которую копируем/название файла который копируем
        ...
        foreach ($arList as $index => $element)
        {
            $arElements[$index]['path'] = $element['path'];     // формируем массив для arResult
            $arElements[$index]['name'] = $element['name'];
            if($element['media_type'] === 'image')
            {
                ...
                $arElements[$index]['preview'] = $absolutepath.$element['name'];
                $forClear[] = $element['name'];
            }
            $arElements[$index]['docviewer'] = $element['docviewer'];
            $arPath[$index] = $this->getPathDirecories($arElements[$index]['path']);
        }
        ...
        $this->arParams['SELECT_FOLDER'] = array_unique($arPath);// arParams тоже буду передавать в шаблон т.к. в arResult морока с этими ключами)))
