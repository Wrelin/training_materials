...
1.1. Он должен подписываться на события OnBeforeIBlockElementAdd и OnBeforeIBlockElementUpdate.  
...
1.3. Добавленный злемент должен попадать в раздел с именем и кодом инфоблока, изменение которого логируется. Если такого раздела нет, он должен создаваться.  
